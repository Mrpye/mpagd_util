...
func Cmd_ImportFont() *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mpagd.LogMessage("Cmd_ImportFont", fmt.Sprintf("Starting import for file: %s", apjFilePath), "info", noColor)
			...
			if outputFilePath == apjFilePath {
				...
				mpagd.LogMessage("Cmd_ImportFont", fmt.Sprintf("Backup created: %s", outputFilePath), "ok", noColor)
			}
			...
			mpagd.LogMessage("Cmd_ImportFont", fmt.Sprintf("Font imported successfully. Updated APJ file saved to %s", outputFilePath), "ok", noColor)
			return nil
		},
	}
