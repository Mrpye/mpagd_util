...
func Cmd_ImportHeader() *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mpagd.LogMessage("Cmd_ImportHeader", fmt.Sprintf("Starting import for file: %s", apjFilePath), "info", noColor)
			...
			if outputFilePath == apjFilePath {
				...
				mpagd.LogMessage("Cmd_ImportHeader", fmt.Sprintf("Backup created: %s", outputFilePath), "ok", noColor)
			}
			...
			mpagd.LogMessage("Cmd_ImportHeader", fmt.Sprintf("Header imported successfully. Updated APJ file saved to %s", outputFilePath), "ok", noColor)
			return nil
		},
	}
