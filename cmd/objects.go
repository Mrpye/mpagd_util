...
func Cmd_ImportObjects() *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mpagd.LogMessage("Cmd_ImportObjects", fmt.Sprintf("Starting import for file: %s", apjFilePath), "info")
			...
			if outputFilePath == apjFilePath {
				...
				mpagd.LogMessage("Cmd_ImportObjects", fmt.Sprintf("Backup created: %s", outputFilePath), "ok")
			}
			...
			mpagd.LogMessage("Cmd_ImportObjects", fmt.Sprintf("Objects imported successfully. Updated APJ file saved to %s", outputFilePath), "ok")
			return nil
		},
	}
