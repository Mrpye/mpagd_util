...
func Cmd_ImportWindows() *cobra.Command {
	...
	var cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mpagd.LogMessage("Cmd_ImportWindows", fmt.Sprintf("Starting import for file: %s", apjFilePath), "ok")
			...
			if outputFilePath == apjFilePath {
				...
				mpagd.LogMessage("Cmd_ImportWindows", fmt.Sprintf("Backup created: %s", outputFilePath), "ok")
			}
			...
			mpagd.LogMessage("Cmd_ImportWindows", fmt.Sprintf("Windows imported successfully. Updated APJ file saved to %s", outputFilePath), "ok")
			return nil
		},
	}
